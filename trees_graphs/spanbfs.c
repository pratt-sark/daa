...
//Spanning Tree from Breadth First Traversal of a connected, undirected, unweighted graph
//Program: 14
...
int ind; //index of Traversal[] array
int *q; //the queue needed for bfs(int)
int k; //front of queue
int k_end; //rear of queue
...
void main()
{
    ...
    printf("\nand displays a spanning tree using Breadth First Traversal.\n\n"); 
...
int ifConnected()
{
    ...
    //initializing front, rear, index of Traversal[] and edge_counter to zero
    ind=0; k=0; k_end=0; edge_counter=0;
    //generating a random node number to start DFS from
    int ran=rand()%n;

    q[k_end++]=ran; //push the starting random node into the queue
